...
Transformed Claudiu from a static Electron window into a working terminal emulator. Added xterm.js in the renderer connected to a real zsh PTY via node-pty through Electron IPC.
...
- **`window._claudiuGetBufferText` for test assertions**: xterm.js renders to canvas, not DOM text. Exposed the buffer API via a global function for Playwright to query. Placed before async IPC calls to avoid timing issues.
...
4. Type `echo HELLO_CLAUDIU` + Enter → buffer contains output — full roundtrip (input → PTY → output)
5. Buffer still contains `HELLO_CLAUDIU` — persistence check
...
- **xterm.js renders to canvas**: Can't query terminal text via DOM selectors (`.xterm-rows > div`). Must use the `terminal.buffer.active` API to read buffer contents. Exposed via `window._claudiuGetBufferText()` for test access.
- **Timing of global function exposure**: The `_claudiuGetBufferText` function must be assigned to `window` before any `await` calls in the renderer init, otherwise a failed IPC call would prevent the assignment and tests would get `not a function`.
